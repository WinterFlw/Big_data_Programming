...
연구의 핵심 흐름 (Before/After 검증):
  Baseline 분석: BERT-base에 SHAP + LIME 적용
           ("개선 전 모델은 어떤 토큰에 주목하고 있었나?" 확인)
  Improved 분석: 개선 모델(+VADER)에 동일 분석 → Before/After 비교
           ("VADER 추가 후 모델의 판단 근거가 어떻게 달라졌나?" 비교)

  참고: XAI는 사전에 세운 가설(VADER가 감성 맥락을 보완)의 "사후 검증" 역할이에요.
  진단 → 개선의 순환 루프가 아니라, 가설 → 실험 → XAI 해석의 과학적 흐름이에요.
