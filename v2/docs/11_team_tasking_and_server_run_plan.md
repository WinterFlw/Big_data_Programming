...
Phase 6: XAI 실행
Phase 7: report/dashboard 생성
Phase 8: 결과 회수와 제출 전 검수
```
...
|---|---|---|---|
| A. Benchmark 담당 | 실제 학습 실행 연결 | `pipeline/runner.py`, `experiment_core.py` | condition x seed 학습 실행 |
| B. Statistics 담당 | 15 seed 통계 검정 | `pipeline/statistics.py` | summary/test CSV |
| C. XAI 담당 | XAI sample/metric/stability | `pipeline/xai.py`, `experiment_xai.py` | XAI CSV/JSON/case |
| D. Report 담당 | 최종 보고서/대시보드 | `pipeline/reporting.py` | final_report, dashboard |
| E. QA/Server 담당 | preflight/status/retry | `pipeline/artifacts.py`, `pipeline/manifest.py` | 서버 실행 체크리스트 |
...
기존 experiment_core.py의 학습 로직을 재사용하되, 모든 산출물은 outputs/experiments/v2_15seed/ 내부에 저장되도록 격리해야 합니다.
```
