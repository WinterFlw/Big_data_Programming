...
| **2. Statistics** | 학습 결과 metrics.json 120개 → mean/std + 핵심 paired test + Cohen's d CSV | 3~5h | 중 |
...
2. Statistics: "15시드 평균/표준편차와 핵심 paired t-test, Cohen's d 결과를
...
## 0.4 정하는 방법 (확정됨 — 2026-05-17)

**5인 분배는 21 문서 §0에서 확정됨**:

```
카드 1 Benchmark      = 정수현 (팀장)
카드 2 Statistics     = 박종화
카드 3 XAI Core       = 차종민
카드 4 XAI Bundle+Report = 조은
카드 5 QA + Author 페어 = 김정훈
```

스왑이 필요하면 §0.4 옛 절차(회의 5분)는 아래 그대로 보존:
...
# 카드 1 — Benchmark Stage  (담당: **정수현** — 21 문서 §0 확정)
...
# 카드 2 — Statistics Stage  (담당: **박종화** — 21 문서 §0 확정)
...
**산출물**: 15 seed mean/std, 핵심 A_B vs D_B paired test, 95% CI, Cohen's d/effect size 컬럼이 들어간 CSV. Holm 보정과 ANOVA는 보조/부록 산출물로 둔다.
...
| 핵심 A_B vs D_B paired test를 손계산으로 교차 검증 | Holm-Bonferroni 보정 코드 (보조 산출물) |
| `paired_tests_holm.csv`에서 A_B:D_B row와 보조 비교 row 확인 | Cohen's dz 계산 |
...
효과 크기 해석: 본문은 paired effect size/Cohen's dz 중심. ANOVA η²는 부록에서만 Cohen 기준(< 0.01 negligible / 0.01~0.06 small / 0.06~0.14 medium / ≥ 0.14 large)을 짧게 언급.
...
- Holm 보정 p와 ANOVA는 여러 조건 비교표의 보조 확인값으로만 둔다.
...
- 금지: 단일 시드 결과로 결론
- 권장: \"핵심 A_B vs D_B paired test row 생성. 평균 차이, CI, effect size 확인 완료\"
...
생성/확인한 산출물: benchmark_summary.csv, paired_tests_holm.csv
통과한 검증: paired_statistics_smoke / A_B vs D_B row / CI·effect size 컬럼 채워짐
남은 위험:
...
# 카드 3 — XAI Core Stage  (담당: **차종민** — 21 문서 §0 확정)
...
# 카드 5 — QA + Server + Author 페어  (담당: **김정훈** — 21 문서 §0 확정)
...
[2. Statistics]     통계 CSV 숫자 직접 검증하는 사람
                    ...
                    AI 부담: statistics.py / paired test / effect size
                    주 3~5h
