...
학습이 끝나면 paired t-test / Holm / Cohen dz / ANOVA / bootstrap CI가 **자동 계산된 표를 받아**, 그 수치를 발표 본문(p17 · p18)에 **어떻게 박을지 정하는** 역할.
...
- p18 평가지표 + 결과 한 문장:
  > "D_B vs A_B Macro F1 차이는 +0.0XX이며 Holm 보정 후 p=0.0XX. BERT 패밀리 2-way ANOVA에서 attention loss 주효과(p=0.0XX, η²=0.XX), VADER 주효과(p=0.0XX, η²=0.XX), 교호작용(p=0.0XX)."
...
├── paired_tests_holm.csv      # Holm 보정 후
...
| p-value 해석 오류 | Holm 보정 안 하면 7쌍에서 false discovery. Holm 보정 후 p값 사용 강제. |
...
- p19 메인 결과 표:
  ...
  - paired_tests_holm.csv → "A_B vs D_B ΔF1 + Holm p"
