...
**D8~D9 (발표 자료 통합 + 부록 슬라이드 ★)**
- markdown → pptx 변환 (또는 직접 작성). 평가 문서 와꾸 (헤드라인 + 8~15 bullet + 근거 수치 + 시사점).
- **부록 슬라이드 첨부 (필수)**: 본문은 paired t-test + CI + effect size 중심으로 두되, 부록에 다음 3장 첨부:
  - 부록 A: `paired_tests_holm.csv` 7쌍 비교 표 (adjusted p-value 포함)
  - 부록 B: `anova_2way_bert.csv` + `anova_2way_roberta.csv` + `anova_3way.csv` 3개 표 (F · p · η² · partial η²)
  - 부록 C: Cohen(1988) 효과 크기 해석 기준 (η² < 0.01 negligible / 0.01~0.06 small / 0.06~0.14 medium / ≥ 0.14 large)
- `final_report.md`에는 `_render_anova_table()`이 이미 "Supplementary ANOVA" 섹션으로 자동 첨부함 → ppt 부록에 그대로 캡처/박기.
...
- Q&A 대비 한 줄 답변 정리:
  > Q: ANOVA 안 보셨나요? → A: 부록 B에 BERT/RoBERTa 2-way + 3-way ANOVA를 효과 크기 함께 첨부했습니다. 본문은 핵심 비교 paired t-test 중심으로 두었습니다.

### 추가 업무 A — 오분류 분석 (Error Analysis)

- 입력: `outputs/.../benchmark/runs/d_b/seed_*/predictions.csv` + `data/dataset.json`
- 작업: D_B가 여전히 틀린 sample을 직접 열어 패턴 분류
  - hate ↔ offensive 혼동 사례 정성 정리
  - Implicit hate (비하어 없는 hate) 미탐 사례
  - Target별 오분류율 (Jewish / African / Women 등)
- 산출물: 발표 부록 슬라이드 1장 + `final_report.md`에 "Error Analysis" 섹션 (markdown 작성)
- 가치: "한계가 뭔가요?" 질문에 정량 + 정성 답변 가능

### 추가 업무 B — Q&A 답변 카드 (v2 결과 기반)

- 입력: 다른 4명이 작성한 본문 + 자동 산출물 (paired t / 4축 XAI / ANOVA)
- 작업: 예상 질문 30~50개 + 답변 카드 작성
  - "ANOVA p > 0.05인데 효과 있다고 주장?" "Holm 보정 후에도 유의?" "왜 15 seed?"
  - "XAI가 인과 증명 아니면 의미 있는가?" "CI/IS/MSS 학술 근거는?"
  - "Cheng 2022와의 차별점은?" "1차 baseline 0.6822와 비교는?"
- 산출물: `docs/Q&A_v2_답변카드.md` 신규 작성. 발표 직전 5인 공유.
- 가치: 발표 당일 5명 누가 질문 받든 즉답. 학부 프로젝트 차별점.

### 추가 업무 C — 추가 시각화 (matplotlib 그림 4장)

- 작업: `scripts/figures.py` 또는 jupyter 노트북으로 그림 4장 작성
  - 그림 1: 8조건 macro F1 boxplot (seed 분산 시각화)
  - 그림 2: Target subgroup F1 heatmap (10개 카테고리 × 조건)
  - 그림 3: SHAP top-5 token 비교 (A_B vs D_B)
  - 그림 4: ANOVA η² + partial η² bar chart (Cohen 기준선 표시)
- 산출물: `outputs/.../figures/*.png` + 발표 본문/부록에 박기
- 가치: 표만 있는 발표 vs 그림 있는 발표 — 시각적 임팩트.

### 시간 부담 곡선 갱신

D0~D5 ★★ (placeholder + p22~p26 골격) → D7~D10 **★★★★★ (메인 + A+B+C 추가)** — 주당 10~15h 예상.
