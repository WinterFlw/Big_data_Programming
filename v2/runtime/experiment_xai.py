...
연구의 핵심 흐름 (v2 15-seed 사후 검증):
  기준 조건 분석: A_B 조건 checkpoint에 SHAP/LIME 적용
  비교 조건 분석: D_B 조건 checkpoint에 동일 분석 적용
...
# v2 canonical 조건명을 우선 사용한다. 뒤쪽 이름들은 오래된 registry 파일을
# 읽어야 하는 경우를 위한 호환 alias이며, 새 v2 실행의 기준 이름은 아니다.
...
# ── 최고 성능 개선 모델 선택 ───────────────────────────────
# v2 D 조건 중 저장된 결과에서 최고 성능 모델을 고릅니다.
...
def _select_baseline_model_name(registry: dict[str, dict[str, Any]]) -> str:
    """Select the reference model; A_B is the v2 canonical baseline."""
...
# v2 결과 registry에서 A_B와 D_B/D_R checkpoint를 준비합니다.
