...
class ExperimentConfig:
    ...
    # ── 반복 실험 시드 ─────────────────────────
    # runtime 단독 실행의 기본 smoke seed입니다. v2_15seed 정식 실행에서는
    # configs/v2_15seed.json의 15개 seed를 pipeline/training_adapter가
    # condition x seed 단위로 주입합니다.
...
def prepare_data(config: ExperimentConfig | None = None, force_refresh: bool = False, force_download: bool = False) -> dict[str, pd.DataFrame]:
    ...
    if SPLITS_PICKLE_PATH.exists() and not force_refresh:
        ...
        print("[data] cached splits use an older schema; rebuilding the v2 data contract", flush=True)
...
def run_transformer_benchmark(
    ...
    # 모든 seed x 모델 조합을 돌려요. 정식 v2_15seed는 pipeline이
    # condition x seed 단위로 실행하므로 이 runtime 단독 벤치마크는 보조 경로입니다.
    records = []
    for seed in seeds:
...
def save_benchmark_artifacts(run_records: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    if not sig_frame.empty:
        ...
        save_text(
            ...
            "⚠️ seed 수가 적은 smoke 결과는 통계 결론이 아니라 실행 검증으로 해석해야 합니다.\n\n"
...
def _artifact_current(path: Path, dependencies: list[Path]) -> bool:
    if not path.exists():
        return False
    artifact_mtime = path.stat().st_mtime
    return all(not dependency.exists() or artifact_mtime >= dependency.stat().st_mtime for dependency in dependencies)
