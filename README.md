...
이 영역은 팀원별 담당 업무와 최종 산출물을 연결하기 위한 공개용 등록 칸입니다. GitHub README에는 전화번호, 이메일, 학번 등 개인정보를 넣지 않습니다.

| 팀원 | 역할 | 주요 기여 | 관련 파일 / 산출물 | 확인 근거 |
|---|---|---|---|---|
| 미정 | 프로젝트 총괄 / 통합 | 파이프라인 설계, 실험 통합, 최종 검수 | `v2/run.sh`, `v2/pipeline/`, `README.md` | 엔드투엔드 실행 로그, 최종 보고서, 커밋 이력 |
| 미정 | 데이터 / 전처리 | 데이터셋 준비, 라벨 매핑, 데이터 분할 검증 | `v2/runtime/`, `v2/configs/v2_15seed.json` | 데이터 분할 정책, 전처리 기록 |
| 미정 | 모델 학습 | BERT/RoBERTa 학습, 시드별 실행 관리, 체크포인트 처리 | `v2/runtime/experiment_core.py`, `v2/pipeline/training_adapter.py` | 완료 실행 기록, 벤치마크 CSV |
| 미정 | 통계 / 결과 분석 | Macro F1 비교, paired test, Holm 보정, ANOVA 해석 | `v2/pipeline/statistics.py`, `v2/outputs/experiments/v2_15seed/benchmark/` | 성능 요약표, 통계 결과표 |
| 미정 | XAI / 보고서 | SHAP/LIME 해석, 근거 정렬 분석, 보고서와 대시보드 자료 | `v2/pipeline/xai*.py`, `v2/docs/04_xai_protocol.md`, `outputs/` | XAI 시각화, 보고서 섹션 |
