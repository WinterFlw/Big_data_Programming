...
# sys: 표준 출력 인코딩을 UTF-8로 맞추는 데 써요.
# Windows 한글 콘솔(cp949)에서 em-dash(—)나 한글 출력 시 UnicodeEncodeError가
# 나는 걸 막아줍니다. (원본은 macOS 기준이라 이 처리가 없었어요.)
import sys
...
# 핵심 파이프라인 함수/경로는 ucam 구조에 맞춰 src/, model/ 로 분리되어 있어요.
# 경로는 src.path, 설정은 src.config, 데이터는 src.data, VADER는 src.vader,
# 학습/벤치마크/상태는 model.train 에 있습니다.
...
from src.utils import CHECKPOINT_DIR, dataframe_to_markdown, remove_tree, save_text
