...
# ──────────────────────────────────────────────
# 프로젝트 디렉토리 구조
# 경로는 src/path.py 한 곳에서 정의하고, 여기서는 import만 해요 (ucam 구조).
# ──────────────────────────────────────────────
...
# (디렉토리 생성은 src/path.py에서 처리해요.)


# ──────────────────────────────────────────────
# 산출물 최신성 검사
# 산출물이 의존하는 입력보다 최신인지 확인해요. data/vader/benchmark 등
# 여러 단계에서 공통으로 쓰이는 헬퍼라 utils로 옮겼어요 (순환참조 방지).
# ──────────────────────────────────────────────
...
def aggregate_run_metrics(run_records: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    예: BERT-base를 seed 42, 52, 62로 3번 돌린 결과 → 평균 ± std로 한 줄 요약
    """
